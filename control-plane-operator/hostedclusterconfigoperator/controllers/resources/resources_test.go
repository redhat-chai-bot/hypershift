...
func TestReconcilePeriodicResync(t *testing.T) {
	...
	// err must be nil; controller-runtime silently ignores RequeueAfter when err != nil.
	g.Expect(err).ToNot(HaveOccurred())
	g.Expect(result.RequeueAfter).To(Equal(resyncInterval),
		"RequeueAfter should equal resyncInterval for periodic resync")
}
