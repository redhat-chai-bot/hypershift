...
func TestSecretBackedStore(t *testing.T) {
	...
	store := &SecretBackedStore{Client: fake.NewClientBuilder().WithScheme(hyperapi.Scheme).Build(), Namespace: owner.Namespace}
...
func TestSecretBackedStoreUsesReaderForServing(t *testing.T) {
	...
	reader := fake.NewClientBuilder().WithScheme(hyperapi.Scheme).Build()
