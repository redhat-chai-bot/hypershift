...
func TestHasTerminatingPods(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "When selector is nil, it should return false",
			selector: nil,
			pods: []corev1.Pod{
		...
		{
			name:           "When no pods exist, it should return false",
			selector:       selector,
			pods:           nil,
			expectedResult: false,
		},
		{
			name:     "When all pods are running, it should return false",
			selector: selector,
			pods: []corev1.Pod{
				{ObjectMeta: v1.ObjectMeta{Name: "pod-1", Namespace: "ns", Labels: map[string]string{"app": "test"}}},
		...
		{
			name:     "When one pod is terminating, it should return true",
			selector: selector,
			pods: []corev1.Pod{
				{ObjectMeta: v1.ObjectMeta{Name: "pod-1", Namespace: "ns", Labels: map[string]string{"app": "test"}}},
		...
		{
			name: "When selector is malformed, it should return error",
			selector: &v1.LabelSelector{
		...
		{
			name:     "When pods do not match the selector, it should return false",
			selector: selector,
			pods: []corev1.Pod{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := HasTerminatingPods(t.Context(), c, "ns", tt.selector, 0)
			if tt.expectError {
				if err == nil {
