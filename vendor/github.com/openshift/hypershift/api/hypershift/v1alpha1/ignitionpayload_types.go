...
// +kubebuilder:validation:XValidation:rule="self.pullSecretName.matches('^[a-z0-9]([-a-z0-9]*[a-z0-9])?([.][a-z0-9]([-a-z0-9]*[a-z0-9])?)*$')",message="pullSecretName must be a valid DNS subdomain name"
// +kubebuilder:validation:XValidation:rule="!has(self.additionalTrustBundle) || self.additionalTrustBundle.name.size() > 0",message="additionalTrustBundle.name must not be empty"
// +kubebuilder:validation:XValidation:rule="!has(self.mgmtConfig) || self.mgmtConfig.all(ref, ref.name.size() > 0)",message="mgmtConfig names must not be empty"
// +kubebuilder:validation:XValidation:rule="!has(self.rolloutConfig) || self.rolloutConfig.all(ref, ref.name.size() > 0)",message="rolloutConfig names must not be empty"
...
// +kubebuilder:validation:XValidation:rule="self.machineConfigServerConfig.name == 'machine-config-server'",message="machineConfigServerConfig.name must be machine-config-server"
...
// +kubebuilder:validation:XValidation:rule="!has(self.cloudConfig) || self.cloudConfig.name in ['azure-cloud-config', 'openstack-cloud-config']",message="cloudConfig.name must identify a supported platform cloud config"
